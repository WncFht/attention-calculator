...
def validate(data):
    ...
    bctx, bsys, bdia = (data.get("bp_context"), data.get("systolic_bp"),
...
_NHC_WEIGHT_URL = ("https://www.nhc.gov.cn/wjw/ylyjs/202412/"
                   "b3d40e0141834897808ce6c9dce76a60.shtml")
_NHC_WAIST_URL = ("https://www.nhc.gov.cn/xcs/c100122/202503/"
                  "27b26d397f4941ff8ff3846ef40026ee.shtml")
_NHC_BP_URL = ("https://www.nhc.gov.cn/cms-search/downFiles/"
               "63f752a17cfd4b4781f744477561866f.pdf")
_NHC_SLEEP_URL = ("https://www.nhc.gov.cn/guihuaxxs/c100133/202503/"
                  "70d5836afe804a858b899ee951a24a13.shtml")
_AHA_HR_URL = ("https://www.heart.org/en/healthy-living/"
...
def _item(key, category, name, display, numeric, unit, status, direction,
          reference, sort_order, url, metadata=None, version="1.0"):
...
def _num_item(key, category, name, value, digits, unit, status, direction,
              reference, sort_order, url, version="1.0"):
    """数值型卡片：numeric/display 均为 round(value, digits)。"""
    rounded = round(value, digits)
    return _item(key, category, name, str(rounded), rounded, unit, status,
                 direction, reference, sort_order, url, version=version)


def _range_item(key, category, name, low, high, digits, unit, reference,
                sort_order, url):
    """区间型卡片：display 'low～high'，numeric 为 null，metadata 带边界。"""
    lo, hi = round(low, digits), round(high, digits)
    return _item(key, category, name, f"{lo}～{hi}", None, unit, None, None,
                 reference, sort_order, url, metadata={"low": lo, "high": hi})


# ---- 提示语（中英成对；触发顺序见 docs/health-notes.md）-------------------

def _tips(fields, bmi_raw, bmi_status, wadj, waist_status, bf_flag,
          bf_out_of_age, watson_flag, rhr_status, bp_direction, sleep_dir):
    """按站端固定顺序生成 tips / tips_en。"""
    tips, tips_en = [], []
    if fields["height_cm"] < HEIGHT_TIP_CM:
        tips.append("身高明显低于成人常见范围；虽然通过了1～250厘米的硬校验，"
                    "但这些成人公式很可能不适用。")
        tips_en.append("The height is well below the range commonly seen in "
    ...
    if wadj is not None:
        amount, gain = wadj
        if gain:
            tips.append(f"按BMI正常范围反算，至少还需增重{amount}公斤可达到"
                        "BMI 18.5；这只是数学反算，不代表个人治疗目标。")
            tips_en.append(f"Based on the BMI reference range, gaining at "
                           f"least {amount} kg would reach a BMI of 18.5. "
                           "This is only a mathematical conversion, not an "
                           "individual treatment target.")
        else:
            tips.append(f"按BMI正常范围反算，至少还需减重{amount}公斤可回到"
                        "BMI 23.9；这只是数学反算，不代表个人治疗目标。")
            tips_en.append(f"Based on the BMI reference range, losing at "
                           f"least {amount} kg would return BMI to 23.9. "
                           "This is only a mathematical conversion, not an "
                           "individual treatment target.")
    if bmi_raw < 18.5:
        tips.append("BMI低于18.5；若近期体重持续下降、食欲差或容易疲劳，"
                    "建议咨询专业人员。")
        tips_en.append("BMI is below 18.5. Consider professional advice if "
                       "weight has continued to fall or if poor appetite or "
                       "fatigue is present.")
    elif bmi_raw >= 24:
        tips.append("BMI达到超重或肥胖参考范围；建议优先采用可持续的饮食与"
                    "运动调整，避免快速节食。")
        tips_en.append("BMI is in the overweight or obesity reference range. "
                       "Prefer sustainable diet and activity changes and "
                       "avoid rapid dieting.")
    else:
        tips.append("BMI处于中国成人正常参考范围，可继续结合腰围、运动习惯和"
                    "身体成分观察趋势。")
        tips_en.append("BMI is within the Chinese reference range for adults. "
    ...
    if waist_status in ("中心型肥胖前期", "达到中心型肥胖界值"):
        tips.append("腰围提示腹部脂肪风险升高；可定期在同一位置复测，并关注"
                    "血压、血糖和血脂。")
        tips_en.append("Waist circumference suggests increased abdominal-fat "
    ...
    if bf_flag:
        tips.append("CUN-BAE给出了不宜解释的结果，身体成分与Cunningham公式"
                    "已停止计算。")
        tips_en.append("CUN-BAE produced a result that should not be "
                       "interpreted, so body-composition and Cunningham "
                       "calculations were stopped.")
    else:
        tips.append("CUN-BAE作为主要体脂估算，Deurenberg作为对照；两者都是"
                    "统计公式，不能替代身体成分测量或医学诊断。")
        tips_en.append("CUN-BAE is the primary body-fat estimate and "
    ...
    if bf_out_of_age:
        tips.append("CUN-BAE原始研究对象为18～80岁；当前年龄超出其验证"
                    "范围，结果仅供查看。")
        tips_en.append("The original CUN-BAE study covered ages 18–80. "
                       "The entered age is outside its validation range, "
                       "so the result is for reference only.")
    if watson_flag:
        tips.append("Watson体水分公式给出了不宜解释的结果，因此体水分及其"
                    "百分比未展示。")
        tips_en.append("The Watson formula produced an uninterpretable "
                       "result, so total body water and its percentage are "
                       "not shown.")
    if rhr_status is not None and rhr_status != "一般成人常见范围":
        tips.append("静息心率受运动水平、情绪、药物和疾病影响；若多次异常或"
                    "伴有不适，应咨询医务人员。")
        tips_en.append("Resting heart rate is affected by fitness, emotion, "
                       "medication, and illness. Seek medical advice if "
                       "readings are repeatedly unusual or accompanied by "
                       "symptoms.")
    if bp_direction == "up":
        tips.append("血压读数高于理想范围；单次测量不能诊断高血压，建议按"
                    "规范在不同时间复测，持续异常时咨询医务人员。")
        tips_en.append("The blood-pressure reading is above the ideal range. "
        ...
        tips.append("血压读数处于偏低范围；若反复出现或伴有头晕、乏力、视物"
                    "模糊或晕厥，建议咨询医务人员。")
        tips_en.append("The blood-pressure reading is in a low range. Seek "
                       "medical advice if this recurs or is accompanied by "
                       "dizziness, weakness, blurred vision, or fainting.")
    if sleep_dir == "down":
        tips.append("平均睡眠时长低于参考范围；除时长外，也应结合醒后精神"
                    "状态、夜间觉醒和作息规律判断睡眠质量。")
        tips_en.append("Average sleep duration is below the reference range. "
                       "Also consider how you feel after waking, night-time "
                       "awakenings, and sleep regularity.")
    elif sleep_dir == "up":
        tips.append("平均睡眠时长高于参考范围；若长期睡得较多仍感到疲倦，"
                    "建议关注睡眠质量并咨询专业人员。")
        tips_en.append("Average sleep duration is above the reference range. "
...
def calculate(f):
    ...
    bf = (-44.988 + 0.503 * age + 3.172 * bmi_raw - 0.026 * bmi_raw**2
          - 0.02 * bmi_raw * age + 0.00021 * bmi_raw**2 * age)
    if not male:
    ...
    janma = (9270 * w / (6680 + 216 * bmi_raw) if male
             else 9270 * w / (8780 + 244 * bmi_raw))
    bsa = math.sqrt(h * w / 3600)

    # --- 腰围 / 腰高比 / 腰臀比 / 派生体型指数 ---
    if waist is not None:
        if male:
            waist_status = ("未达到中心型肥胖前期界值" if waist < 85 else
                            "中心型肥胖前期" if waist < 90 else
                            "达到中心型肥胖界值")
        else:
            waist_status = ("未达到中心型肥胖前期界值" if waist < 80 else
                            "中心型肥胖前期" if waist < 85 else
                            "达到中心型肥胖界值")
        waist_dir = None if waist_status == "未达到中心型肥胖前期界值" else "up"
        whtr_raw = waist / h
        whtr = round(whtr_raw, 3)
        whtr_status = ("低于0.5参考界值" if whtr_raw < 0.5
                       else "达到或超过0.5参考界值")
        ...
        bri = round(364.2 - 365.5 * math.sqrt(
            1 - ((waist / 100 / (2 * math.pi)) ** 2) / (0.5 * h_m) ** 2), 2)
    ...
    if waist is not None and hip is not None:
        whr_raw = waist / hip
        whr = round(whr_raw, 3)
        whr_status = ("低于参考界值" if whr_raw < whr_limit
                      else "达到或超过参考界值")
    ...
    tbw = (2.447 - 0.09516 * age + 0.1074 * h + 0.3362 * w if male
    ...
    blood = (0.3669 * h_m**3 + 0.03219 * w + 0.6041 if male
             else 0.3561 * h_m**3 + 0.03308 * w + 0.1833)

    # --- 代谢 ---
    mifflin = 10 * w + 6.25 * h - 5 * age + (5 if male else -161)
    harris = (88.362 + 13.397 * w + 4.799 * h - 5.677 * age if male
    ...
    if f["bp_context"] is not None:
        ...
        sys_status = ("低于90参考值" if sys_bp < 90 else
                      "理想范围" if sys_bp < 120 else "高于理想范围")
        sys_dir = "down" if sys_bp < 90 else "up" if sys_bp >= 120 else None
        dia_status = ("低于60参考值" if dia_bp < 60 else
                      "理想范围" if dia_bp < 80 else "高于理想范围")
    ...
    items = [
        _num_item("bmi", "body", "BMI", bmi_raw, 1, "kg/m²", bmi_status,
                  bmi_dir, "中国成人18.5～23.9", 10,
                  "https://www.cdc.gov/bmi/about/index.html"),
        _range_item("target_weight", "body", "BMI正常区间对应体重",
                    tw_low, tw_high, 1, "kg", "仅为BMI反算区间", 20,
                    _NHC_WEIGHT_URL),
    ]
    if wadj is not None:
        amount, gain = wadj
        items.append(_item(
            "weight_adjustment", "body", "达到BMI正常范围所需调整",
            str(amount), amount, "kg",
    ...
    bf_ref = ("男性20%～25%提示偏高，≥25%为报警值" if male
              else "女性25%～30%提示偏高，≥30%为报警值")
    items += [
        _num_item("body_fat_cun_bae", "body", "CUN-BAE估算体脂率", bf, 1, "%",
                  bf_status, bf_dir, bf_ref, 30, _CUNBAE_URL),
        _num_item("body_fat_deurenberg", "body", "Deurenberg对照体脂率",
                  deur, 1, "%", deur_status, deur_dir, "简化公式对照值", 35,
                  _DEURENBERG_URL),
    ]
    if fat_mass is not None:
        items.append(_num_item("fat_mass", "body", "估算脂肪重量", fat_mass, 1,
                               "kg", None, None, "由估算体脂率推算", 40,
                               _CUNBAE_URL, version="2.0"))
    if ffm is not None:
        items.append(_num_item("ffm", "body", "估算去脂体重", ffm, 1, "kg",
                               None, None, "由估算体脂率推算", 50, _CUNBAE_URL,
                               version="2.0"))
    items += [
        _num_item("janma_lbm", "body", "Janmahasatian瘦体重", janma, 1, "kg",
                  None, None, "公式估算", 60,
                  "https://pmc.ncbi.nlm.nih.gov/articles/PMC3833312/"),
        _num_item("bsa_mosteller", "body", "体表面积 BSA", bsa, 2, "m²",
                  None, None, "没有统一健康范围", 70,
                  "https://pubmed.ncbi.nlm.nih.gov/3657876/"),
    ]
    if waist is not None:
        items += [
            _num_item("waist_assessment", "body", "腰围判断", waist, 1, "cm",
                      waist_status, waist_dir,
                      "男性前期85、中心型90厘米" if male
                      else "女性前期80、中心型85厘米", 80, _NHC_WAIST_URL),
            _num_item("whtr", "body", "腰高比 WHtR", waist / h, 3, "",
                      whtr_status, whtr_dir, "成人简便界值0.5", 90,
                      "https://pubmed.ncbi.nlm.nih.gov/20819243/"),
        ]
    if waist is not None and hip is not None:
        items.append(_num_item(
            "whr", "body", "腰臀比 WHR", whr_raw, 3, "", whr_status, whr_dir,
            "男性低于0.90" if male else "女性低于0.85", 100,
            "https://www.who.int/publications/i/item/9789241501491"))
    if waist is not None:
        items += [
            _num_item("rfm", "body", "RFM估算体脂率",
                      64 - 20 * h / waist + (0 if male else 12), 1, "%",
                      None, None, "身高和腰围估算", 110,
                      "https://pubmed.ncbi.nlm.nih.gov/30030479/"),
        ]
    if hip is not None:
        items.append(_num_item("bai", "body", "BAI身体肥胖指数",
                               hip / h_m**1.5 - 18, 1, "", None, None,
                               "个体误差可能较大", 120,
                               "https://pubmed.ncbi.nlm.nih.gov/26262696/"))
    if waist is not None:
        items += [
            _num_item("absi", "body", "ABSI体型指数",
                      (waist / 100) / (bmi_raw ** (2 / 3) * math.sqrt(h_m)),
                      4, "", None, None, "原始值需结合人群标准化", 130,
                      "https://pubmed.ncbi.nlm.nih.gov/22815707/"),
            _num_item("bri", "body", "BRI身体圆度指数",
                      364.2 - 365.5 * math.sqrt(
                          1 - ((waist / 100 / (2 * math.pi)) ** 2)
                          / (0.5 * h_m) ** 2),
                      2, "", None, None, "无统一通用健康界值，用于体型比较",
                      140, "https://pmc.ncbi.nlm.nih.gov/articles/PMC3692604/"),
            _num_item("conicity", "body", "锥度指数",
                      (waist / 100) / (0.109 * math.sqrt(w / h_m)), 3, "",
                      None, None, "无统一通用健康界值", 150,
                      "https://www.sciencedirect.com/science/article/pii/"
                      "S2174204917301253"),
        ]
    if not watson_flag:
        items += [
            _num_item("watson_tbw", "body", "Watson估算体水分", tbw, 1, "L",
                      None, None, "人体测量公式估算", 160, _WATSON_URL),
            _num_item("body_water_pct", "body", "估算体水分占比",
                      tbw / w * 100, 1, "%", None, None,
                      "由Watson体水分除以体重推算", 170, _WATSON_URL),
        ]
    items.append(_num_item(
        "blood_volume_nadler", "body", "Nadler估算血容量", blood, 2, "L",
        None, None, "人体测量公式估算，不用于医疗决策", 180,
        "https://pubmed.ncbi.nlm.nih.gov/21936146/"))
    items += [
        _num_item("mifflin_ree", "metabolism", "Mifflin静息代谢", mifflin, 0,
                  "kcal/天", None, None, "静息状态估算", 210,
                  "https://pubmed.ncbi.nlm.nih.gov/2305711/"),
        _num_item("harris_ree", "metabolism", "Harris–Benedict", harris, 0,
                  "kcal/天", None, None, "对照公式", 220,
                  "https://pubmed.ncbi.nlm.nih.gov/11817239/"),
    ]
    if cunningham is not None:
        items.append(_num_item(
            "cunningham_ree", "metabolism", "Cunningham",
            500 + 22 * (w - bf * w / 100), 0, "kcal/天", None, None,
            "基于估算去脂体重", 230, "https://pubmed.ncbi.nlm.nih.gov/7435418/",
            version="2.0"))
    items.append(_num_item("tdee", "metabolism", "每日总能量 TDEE",
                           mifflin * f["pal"], 0, "kcal/天",
                           f"PAL {f['pal']}", None, "没有统一健康范围", 240,
                           "https://www.fao.org/4/y5686e/y5686e07.htm"))
    if rhr is not None:
        items.append(_item("resting_hr", "heart", "静息心率", str(rhr), rhr,
                           "次/分钟", rhr_status, rhr_dir,
                           "多数成人约60～100", 310, _AHA_HR_URL))
    items += [
        _num_item("max_hr_tanaka", "heart", "Tanaka最大心率", max_hr_raw, 0,
                  "次/分钟", None, None, "年龄公式估算", 320,
                  "https://pubmed.ncbi.nlm.nih.gov/11153730/"),
        _range_item("moderate_hr", "heart", "中等强度心率", mod_lo, mod_hi, 0,
                    "次/分钟", "最大心率50%～70%", 330, _AHA_HR_URL),
        _range_item("vigorous_hr", "heart", "较高强度心率", vig_lo, vig_hi, 0,
                    "次/分钟", "最大心率70%～85%", 340, _AHA_HR_URL),
    ]
    if rhr is not None:
        items += [
            _range_item("karvonen_hr", "heart", "Karvonen目标心率",
                        karv_lo, karv_hi, 0, "次/分钟",
                        "心率储备的50%～70%", 350, _CDC_KARVONEN_URL),
            _num_item("heart_rate_reserve", "heart", "心率储备", hrr_raw, 0,
                      "次/分钟", None, None, "估算最大心率减静息心率", 360,
                      _CDC_KARVONEN_URL),
            _num_item("estimated_vo2max", "heart", "估算最大摄氧量",
                      15.3 * max_hr_raw / rhr, 1, "ml/kg/min", None, None,
                      "使用估算最大心率，误差会叠加", 370,
                      "https://doi.org/10.1007/s00421-003-0988-y"),
        ]
    items.append(_range_item("protein_range", "nutrition", "运动人群蛋白质参考",
                             protein_lo, protein_hi, 0, "g/天",
                             "健康运动人群1.4～2.0 g/kg/天", 410,
                             "https://pubmed.ncbi.nlm.nih.gov/28642676/"))
    if f["exercise_type"] is not None:
        items.append(_item(
            "exercise_kcal", "exercise", "本次运动估算热量", str(kcal), kcal,
            "kcal", None, None,
            f"{label} × {f['exercise_minutes']}分钟", 510,
            "https://pacompendium.com/adult-compendium/",
            metadata={"exercise_type": f["exercise_type"], "met": met,
                      "minutes": f["exercise_minutes"]}, version="2.0"))
    if f["bp_context"] is not None:
        items += [
            _item("systolic_bp", "vitals", "收缩压", str(sys_bp), sys_bp,
                  "mmHg", sys_status, sys_dir,
                  f"{ctx_label}；理想值低于120", 610, _NHC_BP_URL,
                  metadata={"measurement_context": f["bp_context"]}),
            _item("diastolic_bp", "vitals", "舒张压", str(dia_bp), dia_bp,
                  "mmHg", dia_status, dia_dir,
                  f"{ctx_label}；理想值低于80", 620, _NHC_BP_URL,
                  metadata={"measurement_context": f["bp_context"]}),
            _item("blood_pressure_category", "vitals", "血压判断", bp_cat,
                  None, "", bp_cat, bp_dir, "单次读数仅供筛查，不能诊断",
                  630, _NHC_BP_URL,
                  metadata={"measurement_context": f["bp_context"]}),
        ]
    if f["sleep_hours"] is not None:
        items.append(_item(
            "sleep_duration", "vitals", "平均每晚睡眠时长",
            str(f["sleep_hours"]), f["sleep_hours"], "小时", sleep_status,
            sleep_dir, sleep_ref, 640, _NHC_SLEEP_URL,
            metadata={"reference_low": sleep_lo, "reference_high": sleep_hi}))

    tips, tips_en = _tips(
        f, bmi_raw, bmi_status, wadj, waist_status, bf_flag, bf_out_of_age,
        watson_flag, rhr_status, bp_dir, sleep_dir)

    return {
        "profile": {"age": age, "height_cm": h, "nickname": f["nickname"],
                    "sex": sex, "weight_kg": w},
        "body": {
            "absi": absi, "bai_pct": bai, "blood_volume_l": round(blood, 2),
            "bmi": bmi, "bmi_status": bmi_status,
            "body_fat_deurenberg_pct": round(deur, 1),
            "body_fat_deurenberg_status": deur_status,
            "body_fat_pct": round(bf, 1), "body_fat_status": bf_status,
            "body_water_pct": water_pct, "bri": bri, "bsa_m2": round(bsa, 2),
            "conicity": conicity, "fat_mass_kg": fat_mass, "ffm_kg": ffm,
            "janma_lbm_kg": round(janma, 1), "rfm_pct": rfm,
            "target_weight_high": tw_high, "target_weight_low": tw_low,
            "waist_status": waist_status, "watson_tbw_l": watson_tbw,
            ...
            "whr": whr, "whr_limit": whr_limit, "whr_status": whr_status,
            "whtr": whtr, "whtr_status": whtr_status,
        },
        "metabolism": {"cunningham_ree": cunningham,
                       "harris_ree": round(harris, 0),
                       "mifflin_ree": round(mifflin, 0), "pal": f["pal"],
                       "tdee": tdee},
        "heart": {"estimated_vo2max": vo2, "heart_rate_reserve": hrr,
                  "karvonen_high": karv_hi, "karvonen_low": karv_lo,
                  "max_hr": max_hr, "moderate_hr_high": mod_hi,
                  "moderate_hr_low": mod_lo, "resting_hr": rhr,
                  "resting_hr_status": rhr_status,
                  "vigorous_hr_high": vig_hi, "vigorous_hr_low": vig_lo},
        "nutrition": {"protein_high_g": protein_hi,
                      "protein_low_g": protein_lo},
        "exercise": {"calories_kcal": kcal, "label": label, "met": met,
                     "minutes": f["exercise_minutes"],
                     "type": f["exercise_type"]},
        "vitals": {"blood_pressure_category": bp_cat,
                   "blood_pressure_direction": bp_dir,
                   "bp_context": f["bp_context"],
                   "bp_context_label": ctx_label,
                   "diastolic_bp": dia_bp, "diastolic_direction": dia_dir,
                   "diastolic_status": dia_status,
                   "sleep_direction": sleep_dir, "sleep_high": sleep_hi,
                   "sleep_hours": f["sleep_hours"], "sleep_low": sleep_lo,
                   "sleep_reference": sleep_ref, "sleep_status": sleep_status,
                   "systolic_bp": sys_bp, "systolic_direction": sys_dir,
