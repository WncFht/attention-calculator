"""Exact checker for zeta_odd kernels: zeta5, zeta7 (exact-mode only).

params encode P = (au + bu·x²)/u multiplying x^{2m+1}(1−x²)^n·ln^r(1/x)/(1+x²);
the true basis moments come from the kernel's own basis() (ln_moment's odd
branch with the η(r+1)→ζ(r+1) scale factor), and the printed claim
s·(power·ζ(r+1) − bound) is solved directly.
"""

from fractions import Fraction

from ..engine import poly_nonneg
from ..kernels.zeta_odd import basis
