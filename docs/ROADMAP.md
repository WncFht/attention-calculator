...
- **站点缺陷（e 型恒等式为假）**：type=e 返回的参数不满足其自身渲染的恒等式。例：e>8/3 → m=0,n=1,a=1,b=-1，渲染 `e-8/3 = ∫(1-x)²eˣdx`，真值 2e-5 ≠ e-8/3；e>3、e>11/4 等**假命题也返回 success**。反推发现 (0,1) 基下全部答案精确满足一组**错误矩** M_a=e/2−1, M_b=5/3−e/2（真值 e−2, 3−e）——其 e 型矩表系统性错误，非偶发 bug。另：e 型对 `comparison='<'` 一律报"方向反了"（即使 e<3 为真），方向检查也坏了。
  - **后果**：e 型（可能还有其他族）的 golden 参数 parity 天然失败——恒等式本身不成立。本方按文章正确数学实现；报告把 e 型不一致单列归因，verify.py 负责给出"站点假恒等式"的量化数字。
  - e>8/3 的正确证明：(m,n)=(1,1), P=x/3，即 ∫x²(1-x)eˣ/3 = e−8/3（已数值验证）。
