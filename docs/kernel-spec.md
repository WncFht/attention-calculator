...
- **exact-only 类型**：`kernels.EXACT_TYPES` 注册无站端对应的新类型（当前 25 型：zeta5/7/9/11、beta4/6/8/10、ln_q_cube、arcsin_q、arsinh_q、gaussint_q、dawson_q、erfiint_q、pi_sqrt2、pi3、pi3_u、pi3_a、li2_q、psi1_q、si_q、cin_q 共 22 个矩核型 + gamma14、gamma34、gamma12 三个复合命题型，逐型规格见下「exact-only 型清单」）。/calculate 仅在 mode=exact 下接受（site 模式仍按站端口径 400）；solve.prove 对不带 exact 的调用报 ValueError；certified_cmp 常数走 `integrand.constant_mpf` 表。/get_integral_image 仍只认站端 29 型——exact 类型的渲染经 kernel 的 render_equation 程序化调用。
