...
- **exact 模式（当前主线）**：数学正确性路径——恒等式在 ℚ 上精确成立、定号有精确证书、方向判定经递增精度认证；每条证明附机器可检 `certificate`；并扩展站端没有的 exact-only 类型（当前 11 型）。方案见 `docs/2026-09-16-math-correctness-plan.md`，当前状态 `docs/2026-09-16-exact-status.md`。
...
- `src/attention_calculator/` — `engine.py`（搜索序/精确消元/定号判定）、`kernels/`（14 族实现：站端 29 型 + exact-only 11 型）、`solve.py`（方向预检 + 分发 + `certified_cmp` 认证方向 + `prove_exact`）、`exact_check/`（每族 ℚ 字典相等复核器，W0）、`certificate.py`（机器可检证书，W5）、`pade.py`（ln/arctan 的 Padé 插值第二证法，W4）、`decompose.py` + `decompose_exact.py`（组合式拆解：站点版 / 可证构造版）、`render.py`（LaTeX 逐字回显）、`server.py` + `templates/` + `static/`（Flask 站点）、`integrand.py`（常数高精值/被积函数重建）、`convex.py`、`health.py`（姊妹应用）
...
- `tests/` — pytest（1141 过 + 21 跳，跳者为语料缺席的判官包装）
...
- API：`POST /calculate`（表单加 `mode=exact` 走正确性路径，响应附 `certificate`）、`GET /get_integral_image`、`POST /decompose_inequality`、`POST /convex/prove`、`POST /health/calculate`
...
.venv/bin/python -m pytest tests/                        # 1141 过 + 21 跳
