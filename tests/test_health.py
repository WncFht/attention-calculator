...
def test_pages_and_route_edges(client):
    ...
    for path in ("/health/en/", "/health/xyz", "/health/static/bg1.png"):
        ...
        assert resp.get_json() == {"error": "服务器内部错误，请稍后再试。",
                                   "ok": False}
...
def test_transport_errors(client):
    ...
    for kw in ({"data": "nickname=x"},                       # form CT
    ...
    resp = client.post("/health/calculate", data=json.dumps(REQ),
...
def test_validation_order_and_messages(client):
    ...
    cases = [
        ...
        ({"exercise_type": "bogus"},
         "请选择页面提供的运动类型。"),                          # 枚举先于成对
        ({"exercise_type": "jogging", "exercise_minutes": 0.5},
         "运动时长应在 1～1440 之间。"),
        ({"exercise_type": "jogging", "exercise_minutes": 30.5},
         "运动时长必须是整数。"),
        ({"exercise_type": "jogging", "exercise_minutes": True},
         "运动时长格式不正确。"),
        ...
        ({"bp_context": "clinic", "systolic_bp": 49.5, "diastolic_bp": 76},
         "收缩压应在 50～300 之间。"),
        ({"bp_context": "clinic", "systolic_bp": 118.7, "diastolic_bp": 76},
         "收缩压必须是整数。"),
...
def test_success_shape_and_record_id(client):
    ...
    res = body["results"]
    assert set(res) == {"profile", "body", "metabolism", "heart",
                        "nutrition", "exercise", "vitals", "items", "tips",
                        "tips_en"}
    assert len(res["items"]) == 19                  # 仅必填 -> 19 张卡片
    assert res["profile"] == {"age": 35, "height_cm": 175.0, "nickname": "Probe",
...
def test_core_formulas(client):
    ...
    res = calc(client, height_cm=170, weight_kg=70,
               pal=1.85).get_json()["results"]
    m = res["metabolism"]
...
def test_watson_flag(client):
    ...
    res = calc(client, age=80, height_cm=250, weight_kg=30,
               sex="female").get_json()["results"]
...
def test_bp_categories(client):
    ...
    def cat(s, d, ctx="clinic"):
        return calc(client, bp_context=ctx, systolic_bp=s,
                    diastolic_bp=d).get_json()["results"]["vitals"]
    ...
    assert cat(160, 100, "home")["blood_pressure_category"] == \
        "达到家庭血压高血压筛查界值"
    assert cat(140, 50, "home")["blood_pressure_category"] == \
        "达到家庭血压高血压筛查界值"
...
def test_items_shape(client):
    ...
    res = calc(client, waist_cm=82, hip_cm=96, resting_hr=62,
               exercise_type="jogging", exercise_minutes=30,
               bp_context="clinic", systolic_bp=118, diastolic_bp=76,
               sleep_hours=7.5).get_json()["results"]
    orders = [i["sort_order"] for i in res["items"]]
    assert orders == sorted(orders)                          # 卡片按 sort_order 序
    assert {i["key"] for i in res["items"]} == {
        "bmi", "target_weight", "body_fat_cun_bae", "body_fat_deurenberg",
        "fat_mass", "ffm", "janma_lbm", "bsa_mosteller", "waist_assessment",
        "whtr", "whr", "rfm", "bai", "absi", "bri", "conicity", "watson_tbw",
        "body_water_pct", "blood_volume_nadler", "mifflin_ree", "harris_ree",
        "cunningham_ree", "tdee", "resting_hr", "max_hr_tanaka", "moderate_hr",
        "vigorous_hr", "karvonen_hr", "heart_rate_reserve", "estimated_vo2max",
        "protein_range", "exercise_kcal", "systolic_bp", "diastolic_bp",
        "blood_pressure_category", "sleep_duration"}
    for i in res["items"]:
        assert set(i) == {"key", "category_key", "sort_order", "display_name",
                          "display_value", "numeric_value", "unit", "status",
                          "direction", "reference", "source_url",
                          "formula_version", "metadata"}
