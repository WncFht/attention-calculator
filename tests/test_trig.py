...
def symbolic_moment_trig_q(m, n, j):
    ...
    S = {
        0: {"sin": sp.Rational(0), "cos": -1 / q_, "1": 1 / q_},
        1: {"sin": 1 / q_**2, "cos": -1 / q_, "1": sp.Rational(0)},
    }
    for k in range(2, m + n + j + 1):
        f = -k * (k - 1) / q_**2
        S[k] = {
            "sin": sp.simplify(k / q_**2 + f * S[k - 2]["sin"]),
...
def test_trig_q_moments_numeric():
    ...
    for k, mom in enumerate(s):
        exact = (
            mom.get("sin_q", F(0)) * mp.sin(qf)
            + mom.get("cos_q", F(0)) * mp.cos(qf)
            + mom.get("1", F(0))
        )
...
def test_trig_pi_n1_article_expansions():
    ...
    den = 8 * q_ * (8 * q_**6 - 28 * q_**5 + 14 * q_**4 + 35 * q_**3 - 28 * q_**2 - 7 * q_ + 6)
    art_sin = (
        (-8 * a - 8 * b) * q_**4
        + (16 * a + 16 * b) * q_**3
        + (2 * a - 10 * b) * q_**2
    ...
    art_cos = ((8 * a + 8 * b) * q_**4 + (-14 * a - 2 * b) * q_**2 + (-9 * a + 9 * b)) / den17
    art_one17 = ((16 * a - 16 * b) * q_**4 + (-40 * a + 40 * b) * q_**2 + (9 * a - 9 * b)) / den17
    assert sp.simplify(cos_coef - art_cos) == 0
...
def test_trig_pi_moments_numeric():
    ...
    for j in range(12):
        ...
        direct = mp.quad(
            lambda x, jj=j: mp.sin(x) ** jj * mp.sin(mp.mpf(3) / 8 * x), [0, mp.pi / 2]
        )
        assert abs(exact - direct) < mp.mpf("1e-30")
...
def test_site_params(kind, power, comp, bound, expected):
    res = (
        trig_q.prove(kind, F(power), comp, F(bound))
        if kind in ("sin_q", "cos_q", "tan_q", "cot_q")
        else trig_pi.prove(kind, F(power), comp, F(bound))
    )
    p = res["parameters"]
    m, n, av, bv, cv, u = expected
    assert (p["m"], p["n"], p["a_val"], p["b_val"], p["c_val"], p["u_val"]) == (m, n, av, bv, cv, u)
...
def numeric_check_trig_pi(kind, power, comp, bound, m, n, au, bu, u, alpha):
    ...
    C = (
        mp.sin(mp.pi * mp.mpf(q_eff.numerator) / q_eff.denominator)
        if kind.startswith("sin")
        else mp.cos(mp.pi * mp.mpf(q_eff.numerator) / q_eff.denominator)
    )

    def integ(x):
        return mp.sin(x) ** m * (1 - mp.sin(x)) ** n * (au + bu * mp.sin(x)) * mp.sin(alf * x)
