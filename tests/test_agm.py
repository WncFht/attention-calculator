...
verification, the floor improvement, tamper rejection, and the leader's
planned wiring (monkeypatched: solve.prove_exact fallback +
certificate.verify_cert dispatch).
"""
...
REAL_VERIFY = certificate.verify_cert


def verify_any(cert: dict) -> bool:
    """certificate.verify_cert plus the leader's planned agm dispatch."""
    if isinstance(cert, dict) and "agm_iter" in cert:
        try:
            ...
            return False
    return REAL_VERIFY(cert)


def verify(cert: dict) -> bool:
    """Verify a cert of either schema (plain agm or composite)."""
    if cert.get("prover") == "composite":
        old = certificate.verify_cert
        certificate.verify_cert = verify_any
        try:
            return gamma_special.verify_cert(cert)
        finally:
            certificate.verify_cert = old
    return agm.verify_cert(agm.cert_parse(agm.cert_jsonable(cert)))
...
def test_gauss_coefficient_and_sign_flip():
    ...
    assert verify(prove("gauss", 2, "<", Fraction(7, 4)))
    assert verify(prove("gauss", -1, "<", Fraction(-4, 5)))
...
def test_varpi_coefficient_and_sign_flip():
    ...
    assert verify(prove("varpi", 2, ">", Fraction(5)))
    assert verify(prove("varpi", -1, "<", Fraction(-25, 10)))
...
def test_solve_fallback_end_to_end(monkeypatch):
    """Mirror the leader's planned wiring: prove_exact except-block fallback
    plus certificate.verify_cert agm dispatch."""
    orig_exact = solve.prove_exact

    def wrapped(module, kind, q, comp, r):
        try:
            return orig_exact(module, kind, q, comp, r)
        except NoSolution:
            if kind in ("varpi", "gauss"):
                ...
                if cert is not None:
                    return {
                        "type": kind,
                        "prover": cert.get("prover", "agm"),
                        "certificate": cert,
                    }
            raise

    monkeypatch.setattr(solve, "prove_exact", wrapped)
    monkeypatch.setattr(certificate, "verify_cert", verify_any)
    ...
    assert verify_any(resp["certificate"])
