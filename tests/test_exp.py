...
@pytest.mark.parametrize("n, ce, c1", [
...
def test_article_e_expansion(n, ce, c1):
    a, b = F(7), F(-3)
    mom = moment_of((a, exp_family.basis_x_moment(n, n, 0, F(1), "e")),
                    (b, exp_family.basis_x_moment(n, n, 1, F(1), "e")))
    assert mom == {"e": F(ce[0]) * a + F(ce[1]) * b,
                   "1": F(c1[0]) * a + F(c1[1]) * b}
...
def test_article_e_q_expansion(q):
    ...
    a, b = F(2), F(-5)
    mom = moment_of((a, exp_family.basis_x_moment(1, 1, 0, q, "e_q")),
...
def test_article_e_pi_expansion(n, ce, c1):
    a, b = F(7), F(-3)
    mom = moment_of((a, exp_family.basis_sin_moment(n, n, 0)),
                    (b, exp_family.basis_sin_moment(n, n, 1)))
...
def test_article_hyperbolic_expansion(q):
    ...
    a, b, c = F(2), F(-5), F(3)
    mom = moment_of((a, hyperbolic.basis_moment(1, 1, 0, q)),
...
def test_basis_sin_moment_numeric(m, n, j):
    ...
    want = mp.quad(lambda x: mp.sin(x) ** (m + j) * (1 - mp.sin(x)) ** n * mp.exp(x),
                   [0, mp.pi])
    assert mp.almosteq(val, want, rel_eps=mp.mpf("1e-30"))
...
def test_moments_match_sympy_integrate():
    ...
    assert sp.simplify(expr.subs(q, 2)
                       - (got.get("e_q", F(0)) * sp.E**2 + got.get("1", F(0)))) == 0


# ---------------------------------------------------------------- prove end-to-end (site-verified)

@pytest.mark.parametrize("kind,power,comp,bound,m,n,solution", [
...
def test_prove_site_parity(kind, power, comp, bound, m, n, solution):
    ...
    assert set(p) == {"m", "n", "a_val", "b_val", "c_val",
                      "au_val", "bu_val", "cu_val", "u_val"}
