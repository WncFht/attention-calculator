...
calc(
    "fid2:e-lt-F0",
    "e< FALSE floateq 6.5e-32",
    type="e",
    power="1",
    comparison="<",
    rational=b(E_TRUE[0][1]),
)
calc(
    "fid2:e-lt-T0",
    "e< TRUE floateq 2.98e-31",
    type="e",
    power="1",
    comparison="<",
    rational=b(E_FALSE[0][1]),
)

# ---- sin_q ----
SIN_TRUE = [
    ("9.6e-33", "8199121568317184/9743795943467975"),
...
calc(
    "fid2:sin-lt-F0",
    "sin< FALSE floateq 9.6e-33",
    type="sin_q",
    power="1",
    comparison="<",
    rational=b(SIN_TRUE[0][1]),
)

# ---- ln_q(2) ----
LN_TRUE = [
    ("1.3e-32", "1554903831458736/2243252046704767"),
...
LN_FALSE = [
    ("3.43e-32", "6507981849218431/9389033140062816"),
...
calc(
    "fid2:ln-lt-T0",
    "ln< TRUE floateq 3.43e-32",
    type="ln_q",
    power="2",
    comparison="<",
    rational=b(LN_FALSE[0][1]),
)
...
calc(
    "fid2:cos-lt-T0",
    "cos< TRUE floateq 1.54e-31",
    type="cos_q",
    power="1",
    comparison="<",
    rational=COS_FALSE[0][1],
)

# ---- zeta3 ----
Z_FALSE = [
    ("1.02e-32", "9890949372930056/8228353705163039"),
    ("1.46e-31", "3228169011047533/2685537600227014"),
]
for i, (g, r) in enumerate(Z_FALSE):
    calc(
        f"fid2:zeta-gt-F{i}",
        f"zeta> FALSE floateq gap -{g}",
        type="zeta3",
        power="1",
        comparison=">",
        rational=b(r),
...
calc(
    "fid2:zeta-gt-T0",
    "zeta> true floateq 2.45e-31",
    type="zeta3",
    power="1",
    comparison=">",
    rational="1144870450278330/952426168236337",
)

# ---- arctan_q(3) ----
calc(
    "fid2:atan-gt-F0",
    "atan3> FALSE floateq 5.5e-31",
    type="arctan_q",
    power="3",
    comparison=">",
    rational="9972443475689970/7984049661000163",
)
calc(
    "fid2:atan-gt-T0",
    "atan3> true floateq 4.36e-28",
    type="arctan_q",
    power="3",
    comparison=">",
    rational="28688140985470/22968046183277",
)

# ---- ln_q_square(2) ----
LNSQ_TRUE = [
    ("1.06e-32", "4646620020445232/9671330777074349"),
    ("4.19e-32", "1585407615904731/3299818233994160"),
    ("1.37e-30", "109602827268961/228123924908131"),
]
LNSQ_FALSE = [
    ("5.67e-33", "3061212404540501/6371512543080189"),
    ("1.71e-31", "1366201961366809/2843570384177898"),
]
for i, (g, r) in enumerate(LNSQ_TRUE[1:]):
    calc(
        f"fid2:lnsq-gt-T{i + 1}",
        f"lnsq> true floateq gap {g}",
        type="ln_q_square",
        power="2",
        comparison=">",
        rational=b(r),
...
calc(
    "fid2:pin-gt-F0",
    "pi_n> FALSE floateq 9.14e-30",
    type="pi_n",
    power="5/2",
    comparison=">",
    rational="2755163350798137/157497139735537",
)
calc(
    "fid2:pin-gt-T0",
    "pi_n> true floateq 4.06e-30",
    type="pi_n",
    power="5/2",
    comparison=">",
    rational="8417507566830365/481181402581438",
)
calc(
    "fid2:pin-gt-T1",
    "pi_n> true floateq 1.58e-26",
    type="pi_n",
    power="5/2",
    comparison=">",
    rational="133169423484989/7612544386176",
)
