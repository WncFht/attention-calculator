...
img(
    "img2:m11-atan",
    "arctan_q m=11：阈值=类型 cap?",
    type="arctan_q",
    m="11",
    coef="1",
    rational="1",
)
img("img2:m10-atan", "arctan_q m=10", type="arctan_q", m="10", coef="1", rational="1")
img("img2:m33-e", "e m=33（若阈值=30 则过大）", type="e", m="33", rational="3")
...
img("img2:n11-atan", "arctan_q n=11", type="arctan_q", n="11", coef="1", rational="1")
...
# -------------------------------------------- image: u 与系数字段
img("img2:u0-e", "e 型 u_val=0（gamma 退化可过，e 呢）", type="e", u_val="0", rational="3")
img(
    "img2:u0-gamma-plain",
    "gamma 型 u_val=0 普通参数",
    type="gamma",
    u_val="0",
    m="2",
    n="4",
...
img("img2:prec-m-comp", "m=abc&comp='='：m 校验 vs comp", m="abc", comparison="=")
img("img2:prec-comp-a", "comp='='&a_val=abc：comp vs a_val", comparison="=", a_val="abc")

# -------------------------------------------- /calculate 校验次序
calc(
    "ord:comp-eq-bad-rat",
    "comp='=' + rational=abc：谁先",
    type="pi",
    power="1",
    comparison="=",
    rational="abc",
...
calc(
    "ord:bad-type-bad-rat",
    "type=foo + rational=abc",
    type="foo",
    power="1",
    comparison="<",
    rational="abc",
...
calc(
    "ord:good-comp-bad-pow",
    "comp 合法 + power=x + rational=abc",
    type="pi",
    power="x",
    comparison="<",
    rational="abc",
...
calc("fmt:rat-tab", "rational 前导 tab", type="pi", power="1", comparison="<", rational="\t22/7")
calc(
    "fmt:rat-newline", "rational 尾随换行", type="pi", power="1", comparison="<", rational="22/7\n"
)
calc(
    "fmt:rat-2slash",
    "rational='22/7/1' 双斜杠",
...
calc("fmt:rat-slash-end", "rational='22/'", type="pi", power="1", comparison="<", rational="22/")
calc("fmt:rat-slash-start", "rational='/7'", type="pi", power="1", comparison="<", rational="/7")
calc(
    "fmt:pow-frac-neg",
    "power='-1/2' 负分数",
    type="e_q",
...
calc(
    "beh:gamma0-lt0b", "gamma 0 < 0：0<0 假", type="gamma", power="0", comparison="<", rational="0"
)
calc(
    "beh:sinh0-lt-big",
    "sinh 0 < 9999999999999999",
    type="sinh_q",
    power="0",
    comparison="<",
...
raw(
    "mth2:delete-image", "/get_integral_image", "DELETE", "query", IMG, "DELETE /get_integral_image"
)
raw("mth2:options-calculate", "/calculate", "OPTIONS", "form", {}, "OPTIONS /calculate")
raw(
    "mth2:get-image-slash",
    "/get_integral_image/",
    "GET",
    "query",
    IMG,
...
raw("mth2:dec-empty", "/decompose_inequality", "POST", "form", {}, "decompose 缺 problem 字段")
raw(
    "mth2:dec-emptystr",
...
def site_request(p):
    if p["method"] == "GET":
        ...
        resp = SESSION.put(BASE + p["endpoint"], data=p["request"], timeout=TIMEOUT)
    elif p["method"] == "DELETE":
        resp = SESSION.delete(BASE + p["endpoint"], params=p["request"], timeout=TIMEOUT)
